...
c  bs24_driver.f
c
c  Driver program and interpolation subroutine for the Bayless and
c  Somerville (2024) Ground Motion Model for Australia (BS24).
c
c  Author:   Thuany Costa de Lima
c            Geoscience Australia
c            thuany.costadelima@ga.gov.au
c
c  Purpose:  Jeff Bayless's bs24_gmm.f contains only a subroutine —
c            it has no program block and cannot be run on its own.
c            This file provides:
c              (1) S24_interp  — the interpolation subroutine that
c                                bs24_gmm.f calls internally
c              (2) BS24_driver — the main program that loops over
c                                a matrix of scenarios and writes
c                                results to a CSV file
c
c  Usage:
c    Compile:  gfortran -O2 -ffixed-line-length-none -o bs24_run
c                       bs24_driver.f bs24_gmm.f
c    Run:      ./bs24_run
